...
//			flagEndNum = endNum + 1;
			if(endNum < startNum){
				...
				startNum = Integer.parseInt(sNum)-1;
				if(!((endNum <= size && endNum > 0) && (startNum >= 0 && startNum < size))){
					JOptionPane.showMessageDialog(null, "输入的行号不在可执行范围内(1~" + size
						+ "), 请重新输入!", "提示", 1);
				}else{
					System.out.println();
					System.out.println("您所要执行的命令区间段为：" + sNum + "~" + eNum + "行");
					this.setVisible(false);
					MRF.playScript(startNum, endNum);
	...
	public Play(MonkeyRecorderFrame m) {
		this();
		MRF = m;
	}
