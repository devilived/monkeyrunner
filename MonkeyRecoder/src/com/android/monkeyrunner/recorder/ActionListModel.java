...
public class ActionListModel extends AbstractListModel {
	...
	private List<Action> testActionList = Lists.newArrayList();
	...
	public void addActionItem(int index, Action a) {
		testActionList = Lists.newArrayList();
		for (Action iterable_element : actionList) {
			testActionList.add(iterable_element);
		}
		for (int i = actionList.size() - 1; i > index; i--) {
			actionList.remove(i);
		}
		actionList.add(a);
		for (int i = index + 1; i < testActionList.size(); i++) {
			actionList.add(testActionList.get(i));
		}
		testActionList = null;
		int newIndex = actionList.size() - 1;
		this.fireIntervalAdded(this, newIndex, newIndex);
